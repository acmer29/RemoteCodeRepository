...
bool testCase::testR5() {
	...
	std::cout << "\n After replace the element name \"Tianyu Qi1\" with a new set of values";
	NoSqlDb::DbElement<std::string> newElement;
	newElement.name("Tianyu Qi1"), newElement.descrip("I am a new element");
	querier.from(db).find("name", "Tianyu Qi1").update(newElement);
	querier.from(db).find("name", "Tianyu Qi1").resultDisplay();
	if (querier.from(db).find("name", "Tianyu Qi1").eval()[0].descrip() != "I am a new element") return false;

	std::cout << "\n After finding the element name \"Tianyu Qi1\" updating its payLoad and description as well as category";
	querier.from(db).find("name: \"Tianyu Qi1\"").update("payLoad: \"Droped CSE 687\", description: \"A student\", category: \"Male, CHINESE, Asian\"");
	querier.from(db).find("name: \"Tianyu Qi1\"").resultDisplay();
	...
	try {
		...
		std::cout << "\n Catch the exception of try editing the name";
...
bool testCase::testR8() {
	...
	std::cout << "\n The persisted XML file can be found in test.xml" << std::endl;
...
bool testCase::testR11() {
	...
	querier.from(db).insert("\"Query\", \"Provide query methods of the database\", \"DbCore.h, DbCore.cpp, DateTime.h, DateTime.cpp, Utilities.h, Utilities.cpp, Query.h, Query.cpp\", \"Query.h, Query.cpp\", \"NoSqlDb\", \"\", \"\", \"\"", false);
	querier.from(db).insert("\"Persistence\", \"Provide persist methods of database \", \"DbCore.h, DbCore.cpp, DateTime.h, DateTime.cpp, Utilities.h, Utilities.cpp, Query.h, Query.cpp, Persistence.h, Persistence.cpp, XmlDocument.h, XmlDocument.cpp\", \"Persistence.h, Persisence.cpp\", \"NoSqlDb\", \"\", \"\", \"\"", false);
	...
	std::cout << "\n The project structure is shown as records in database, the XML file can be found in Project package structure.xml" << std::endl;
	...
	querier.from(db).resultDisplay();
	if (querier.from(db).hasCategory("NoSqlDb") == false) return false;
